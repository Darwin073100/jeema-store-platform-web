...
function ConfigInformationCloud({workspace}:Props) {
    ...
    return (
        <article className="bg-white rounded-2xl p-4 flex flex-col gap-4 text-gray-600">
            <div className="flex items-center justify-between gap-4">
                ...
                <Badge size="lg" type={workspace?.establishment?.enrollmentKey? 'blue': 'red'}>{workspace?.establishment?.enrollmentKey? 'Activo': 'Inactivo'}</Badge>
            </div>
            <div className="flex gap-4 justify-between">
                <div>
                    <div className="bg-gray-100 p-4 rounded-2xl flex gap-2">
                        <span>Clave de inscripción activa: </span>
                        <Badge type={workspace?.establishment?.enrollmentKey? 'blue': 'red'}>{workspace?.establishment?.enrollmentKey? workspace?.establishment.enrollmentKey: 'No registrado'}</Badge>
                    </div>
