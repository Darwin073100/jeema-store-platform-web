...
export class InventorySalePriceManyVO extends ValueObject<Prop> {
  ...
  public static create(name?: number|null): InventorySalePriceManyVO {
    if (name && name < 0) { // Un ejemplo de límite, puedes ajustar
      throw new InvalidInventoryException('El precio de venta por mayoreo no puede ser negativo.');
    }
    // Podrías añadir más validaciones: caracteres especiales, formato, etc.

    return new InventorySalePriceManyVO({ value:name });
  }
