...
export class InventoryMinStockBranchVO extends ValueObject<Prop> {
  ...
  public static create(name?: number|null): InventoryMinStockBranchVO {
    if (name && name < 0) { // Un ejemplo de límite, puedes ajustar
      throw new InvalidInventoryException('El stock minimo por sucursal no puede ser negativo.');
    }
    // Podrías añadir más validaciones: caracteres especiales, formato, etc.

    return new InventoryMinStockBranchVO({ value:name });
  }
