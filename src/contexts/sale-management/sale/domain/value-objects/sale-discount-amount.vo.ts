...
export class SaleDiscountAmountVO extends ValueObject<Prop> {
  ...
  public static create(value: number): SaleDiscountAmountVO {
    if (value && value < 0) {
      throw new SaleInvalidException('El descuento para la venta no puede ser negativo.');
    }
    // Podrías añadir más validaciones: caracteres especiales, formato, etc.

    return new SaleDiscountAmountVO({ value });
  }
