...
export class SaleTotalAmountVO extends ValueObject<Prop> {
  ...
  public static create(value: number): SaleTotalAmountVO {
    if (value && value < 0) {
      throw new SaleInvalidException('El total de la venta no puede ser negativo.');
    }
    // Podrías añadir más validaciones: caracteres especiales, formato, etc.

    return new SaleTotalAmountVO({ value });
  }
