...
export function getTicket54DocumentDefinition(sale: SaleEntity): TDocumentDefinitions {
	const details = sale.saleDetails ? sale.saleDetails.flatMap(item => [
		...
		[
			{ text: item.quantity.toString(), fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify')},
			{ text: '', fontSize: 8 },
			{ text: `$${item.unitPriceAtSale?.toFixed(2) ?? '0.00'}`, fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify') },
			{ text: `$${item.discountItem?.toFixed(2) ?? '0.00'}`, fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify') },
			{ text: `$${((item.subtotalItem ?? 0) + (item.discountItem ?? 0)).toFixed(2)}`, fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify') },
			{ text: `$${item.subtotalItem?.toFixed(2) ?? '0.00'}`, fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify'), bold: true }
		],
		[
			{ colSpan: 6, text: '-----------------------------------------------------------', fontSize: 8, alignment: 'left' as ('left' | 'right' | 'center' | 'justify'), bold: true },
			'', '', '', '', ''
		],
	]) : [];
