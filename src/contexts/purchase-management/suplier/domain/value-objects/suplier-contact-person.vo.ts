...
export class SuplierContactPersonVO extends ValueObject<Prop> {
   ...
   *
   * @param contactPerson El valor del nombre a encapsular.
   * @returns Una nueva instancia de Name.
   * @throws Error si el nombre es inválido (ej. vacío, muy largo).
   */
  public static create(contactPerson: string | null): SuplierContactPersonVO {
    if(!contactPerson) return new SuplierContactPersonVO({value: null}); 
    if(contactPerson.trim().length === 0) return new SuplierContactPersonVO({value: null});
    if (contactPerson.length > 150) { // Un ejemplo de límite, puedes ajustar
      throw new InvalidSuplierException('El nombre de la persona de contacto no debe ser mayor a 150 caracteres.');
    }
    if (contactPerson.length < 3) { // Un ejemplo de límite, puedes ajustar
    ...
    return new SuplierContactPersonVO({ value:contactPerson });
  }
