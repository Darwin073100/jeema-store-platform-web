...
import { ICashSession } from "../../presentation/interfaces/ICashSession";
...
export class CashSessionMapper {
    ...
    public static toIResponse(domainEntity: CashSessionEntity): ICashSession {
        const dto: ICashSession = {
            cashSessionId: domainEntity.cashSessionId,
            cashRegisterId: domainEntity.cashRegisterId,
            employeeId: domainEntity.employeeId,
            startTime: domainEntity.startTime,
            ...
            updatedAt: domainEntity.updatedAt,
            deletedAt: domainEntity.deletedAt,
            cashRegister: domainEntity.cashRegister ? CashRegisterMapper.toIResponse(domainEntity.cashRegister): null,
            employee: domainEntity.employee ? EmployeeMapper.toIResponse(domainEntity.employee) : null,
            transactions: domainEntity.transactions ? domainEntity.transactions.map(transaction => TransactionMapper.toIResponse(transaction)) : [],
        }
        return dto;
    }
}