...
import { UpdatePrinterConfigurationCommand, UpdatePrinterConfigurationUseCase } from "../../application/use-cases/update-printer-configuration.use-case";
import { Result } from "@/shared/lib/utils/result";
import { PrinterConfigurationMapper } from "../../application/mappers/printer-configuration.mapper";
import { handleError } from "@/shared/infrastructure/http/handlers/handleError";

export async function updatePrinterConfigurationAction(command: UpdatePrinterConfigurationCommand) {
    unstable_noStore();
    try {
        const repository = await TypeormPrinterConfigurationRepository.create();
        const useCase = new UpdatePrinterConfigurationUseCase(repository);
        ...
        return {
            ...handleError(error, 'updatePrinterConfigurationAction'),
        };
    }
}
