...
export const updateLotSchema = yup.object().shape({
    lotId: yup
        ...
        .matches(/^\d+$/, 'El ID del lote debe ser una cadena numérica.'),

    productId: yup
        .string()
        .required('El ID del producto es obligatorio.')
        .matches(/^\d+$/, 'El ID del producto debe ser una cadena numérica.'),
...
const useUpdateLotModal = () => {
    ...
    const resetFormUpdateLot = () => {
        ...
        clearErrors(['expirationDate', 'initialQuantity', 'lotId', 'lotNumber', 'manufacturingDate', 'productId',
